...
```
dinotty-plugins/
  sources/                   # One source declaration per plugin ID
...
commands. Do not add this plugin to `sources/cloudflare-quick-tunnel.yaml` until the native artifact
...
5. Add `sources/<plugin-id>.yaml` with `repo` and optional `subdir`; the file name must match `plugin.json.id`

## Marketplace source declarations

Each published plugin has one strict declaration under `sources/`:

```yaml
repo: xichan96/dinotty-plugins
subdir: json-formatter
```

`subdir` is omitted for a repository-root plugin. Pull requests may add or
remove exactly one declaration file; the marketplace gate validates the
manifest, release tag, repository identity, and archive before merge. The
old `repositories.txt` input file is retired and must not be reintroduced.
`registry.json` remains as a compatibility projection for older clients. It is
generated by the website-backed registry projection workflow; ordinary pull
requests must not edit it directly.
